...
local Tabs = {
    ...
    ["UI Settings"] = Window:AddTab("UI设置", "sliders-horizontal"),
}
...
local MainESPGroup = Tabs.Visuals:AddLeftGroupbox("主要ESP", "eye") do
...
local ESPGroupBox = Tabs.Visuals:AddLeftGroupbox("捉迷藏ESP", "search") do
...
local ESPSettingsGroupBox = Tabs.Visuals:AddRightGroupbox("ESP设置", "sliders") do
...
local FunGroupBox = Tabs.Main:AddLeftGroupbox("有趣的", "zap") do
...
local GreenLightRedLightGroup = Tabs.Main:AddLeftGroupbox("红灯 / 绿灯", "traffic-light") do
    ...
    GreenLightRedLightGroup:AddButton("完整红灯 / 绿灯", function()
    ...
    GreenLightRedLightGroup:AddButton("带来受伤的玩家", function()
...
local DalgonaGameGroup = Tabs.Main:AddLeftGroupbox("抠糖饼游戏", "circle") do
    DalgonaGameGroup:AddButton("完成抠糖饼游戏", function()
...
local TugOfWarGroup = Tabs.Main:AddLeftGroupbox("拉拔河", "rope") do
...
local MingleGroup = Tabs.Main:AddLeftGroupbox("混合", "users") do
...
local GlassBridgeGroup = Tabs.Main:AddLeftGroupbox("玻璃桥", "bridge") do
    GlassBridgeGroup:AddButton("完整的玻璃桥游戏", function()
    ...
    GlassBridgeGroup:AddButton("揭示玻璃桥", function()
        if not workspace:FindFirstChild("GlassBridge") then
            Script.Functions.Alert("Game not running")
            return
        end
...
local InformationGroup = Tabs.Main:AddRightGroupbox("信息", "info") do
    InformationGroup:AddLabel("欢迎使用墨水游戏汉化脚本!")
    InformationGroup:AddLabel("确保加入我的qq群 \n 服务器更新！")
    InformationGroup:AddLabel("🤓请输入文本🤓")
    InformationGroup:AddButton("QQ群:1015268136", Script.Functions.JoinDiscordServer)
    InformationGroup:AddButton("关闭脚本", function() Library:Unload() end)
end
...
local HideAndSeekGroup = Tabs.Main:AddRightGroupbox("捉迷藏", "search") do
    ...
    HideAndSeekGroup:AddButton("传送到安全藏身处", function()
        if Script.GameState ~= "HideAndSeek" then 
            Script.Functions.Alert("Game not running!")
            return
        end
...
local RebelGroup = Tabs.Main:AddRightGroupbox("叛乱者", "sword") do
    ...
    RebelGroup:AddButton("带来所有的警卫", function()
        Script.Functions.PivotRebelGuardsToPlayer()
    end)

    RebelGroup:AddToggle("GunMods", { Text = "枪械改装件", Default = false })
...
local AimbotGroup = Tabs.Main:AddRightGroupbox("瞄准机器人", "bow-arrow") do
    AimbotGroup:AddToggle("GuardAimbot", { Text = "守卫自瞄器", Default = false })
    AimbotGroup:AddToggle("PlayerAimbot", { Text = "玩家自瞄器", Default = false })
    AimbotGroup:AddToggle("AimbotWallcheck", { Text = "墙检查", Default = true })
...
local AutoShootGroup = Tabs.Main:AddRightGroupbox("自动射击", "target") do
    ...
    AutoShootGroup:AddToggle("AutoShootWallcheck", { Text = "墙检查", Default = true })
...
local EmotesGroup = Tabs.Misc:AddRightGroupbox("杂类", "smile") do
    ...
    EmotesGroup:AddButton("开始动作", function()
    ...
    EmotesGroup:AddButton("停止动作", function()
        if Script.Temp.EmoteTrack and typeof(Script.Temp.EmoteTrack) == "Instance" and Script.Temp.EmoteTrack:IsA("AnimationTrack") then
            pcall(function() Script.Temp.EmoteTrack:Stop() end)
            Script.Temp.EmoteTrack = nil
        end
...
local MiscGroup = Tabs.Misc:AddLeftGroupbox("杂类", "wrench") do
    ...
    MiscGroup:AddDivider()
    MiscGroup:AddButton("固定焦点摄影机", function()
    ...
    MiscGroup:AddButton("重置相机 \n [可能会弄坏你的相机！]", Script.Functions.FixCamera)
...
local PerformanceGroupBox = Tabs.Misc:AddRightGroupbox("完成", "gauge") do
...
local Useful = Tabs.Other:AddRightGroupbox("有用的东西", "star") do
...
local AntiDeathGroup = Tabs.Other:AddRightGroupbox("反死亡", "skull") do
...
local InteractionGroup = Tabs.Other:AddLeftGroupbox("人机交互专家", "hand-pointer") do
...
local PlayerAttachGroupBox = Tabs.Other:AddLeftGroupbox("玩家连接", "users-round") do
...
local CreditsGroup = Tabs["UI Settings"]:AddRightGroupbox("汉化人员名单", "heart")
...
MenuGroup:AddLabel("Menu bind"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = false, Text = "菜单键绑定" })
MenuGroup:AddButton("Join Discord Server", Script.Functions.JoinDiscordServer)
MenuGroup:AddButton("Unload", function() Library:Unload() end)

CreditsGroup:AddLabel("汉化者:砂狼白子")
CreditsGroup:AddLabel("汉化者qq:1536618983")
...
InformationGroup:AddButton("保存设置", function()
    local suc, err = pcall(function()
...
InformationGroup:AddButton("重置设置", function()
    if approved then
        pcall(function()
